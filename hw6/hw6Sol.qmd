...
en_pred <- predict(final_fit_en, new_data = NYSE_test)$.pred

# calculate test rsq
rsq_en <- cor(NYSE_test$log_volume, en_pred) %>% round(2)
rsq_en
```
...
# predict the test case
rf_pred <- predict(final_fit_rf, new_data = NYSE_test)$.pred

# calculate test rsq
rsq_rf <- cor(NYSE_test$log_volume, rf_pred) %>% round(2)
```
...
predictions <- predict(final_fit_gb, new_data = NYSE_test) %>% bind_cols(NYSE_test)
