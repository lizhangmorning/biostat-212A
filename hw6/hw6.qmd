...
seq(1, 30) %>%
  map(function(x) {
    cor(NYSE$log_volume, lag(NYSE$DJ_return, x),
  ...
  geom_segment(mapping = aes(xend = lag, yend = 0)) +
  ggtitle(
    "The autocorrelation function for `log volume` and lagged `DJ_return`"
  )
```
...
```{r}
# Drop beginning trading days which lack some lagged features
NYSE_other <- NYSE %>%
  filter(train == 'TRUE') %>%
  select(-train) 
dim(NYSE_other)
...
library(yardstick)
# cor(NYSE_test$log_volume, NYSE_test$log_volume_lag1) %>%
r2_test_strawman <- rsq_vec(NYSE_test$log_volume, lag(NYSE_test$log_volume, 1)) 
